"""
page_terminal_session / terminal_session_page.py
────────────────────────────────────────────────────────────────────────────────
Terminal Session page for pagepack_chilos.

Shell contract:
    page = PageTerminalSession(parent_widget)
...
Storage (all paths relative to pack root, created at runtime):
    /pagepack_chilos/terminalhistory/   — .md session log files
    /pagepack_chilos/linuxcommands/     — .json command records
    /pagepack_chilos/task_options.json  — persisted task/subtask lists
...
class PageTerminalSession:
    """
    Terminal Session page for pagepack_chilos.

    Shell contract (Guichi loader):
        page = PageTerminalSession(parent_frame)
        page.build(parent)   # also: create_widgets / mount / render
    """

    PAGE_NAME   = "terminal_session"
    ...
    def _build_notebook(self):
        ...
        session_tab.rowconfigure(2, weight=1)
    ...
    def _auto_find_root(self):
        ...
        for candidate in [page_file.parent, *page_file.parents]:
            if candidate.name == "pagepack_chilos":
                self._set_root(str(candidate)); return
            if (candidate / "pagepack_chilos").is_dir():
                self._set_root(str(candidate / "pagepack_chilos")); return
        cwd   = os.getcwd()
        probe = os.path.join(cwd, "pagepack_chilos")
        if os.path.isdir(probe):
            self._set_root(probe); return
        if os.path.basename(cwd) == "pagepack_chilos":
            self._set_root(cwd); return
        self._set_status("Root not found \u2014 use Choose Root.")

    def _choose_root(self):
        d = filedialog.askdirectory(title="Select pagepack_chilos directory")
        if d:
            self._set_root(d)
    ...
    def _build_session_tab(self, parent):
        ...
        # Row 2 (expand): HPanedWindow — queue | session view
        paned = ttk.PanedWindow(parent, orient="horizontal")
        paned.grid(row=2, column=0, sticky="nsew", padx=4, pady=(2, 2))
        ...
        cmd_f = ttk.LabelFrame(parent, text="Command Input  (Enter / Numpad Enter = Send  •  Ctrl+Enter = Add to Queue)",
                                padding=(6, 4))
        cmd_f.grid(row=3, column=0, sticky="ew", padx=4, pady=(2, 4))
        cmd_f.columnconfigure(0, weight=1)
    ...
    def _save_session_log(self):
        ...
        for ci, chunk in enumerate(chunks):
            ...
            header_lines = [
                "---",
                "schema_version: 1",
                "pack: pagepack_chilos",
                "page: terminal_session",
...
# ─────────────────────────────────────────────────────────────────────────────
# Module-level factory helpers (used by PageTerminalSession)
# ─────────────────────────────────────────────────────────────────────────────
